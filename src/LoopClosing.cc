...
void LoopClosing::Run()
{
    ...
    while(1)
    {
        ...
        if(CheckNewKeyFrames())
        {
            ...
            mpiGPSFusioner->inputVO(time9 * mpCurrentKF->mTimeStamp,mpCurrentKF->GetPose());
            for(int m = 0 ; m < mviGPSTimestamps.size() ; m ++ )
            {
                double t_frame = time9 * mpCurrentKF->mTimeStamp;
                ...
                {
                    mpiGPSFusioner->inputiGPS(t_frame,mviGPSPosition[m]);
                    binputiGPS = true;
                }
            ...
            if(!binputiGPS)
            {
                for(int m = 0 ; m < mviGPSTimestamps.size()-1 ; m ++ )
                {
                    ...
                    if(t_frame > t_iGPS  && t_frame < t_iGPS_next && t_iGPS_next-t_iGPS<10)
                    {
                        ...
                        mpiGPSFusioner->inputiGPS(t_frame,iGPSPosition);
                        break;
                    }
