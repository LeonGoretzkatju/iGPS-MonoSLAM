...
{  
    ...
    vector<cv::Point3f> iGPSPosition;
    vector<double> vTimestampsiGPS;
    LoadiGPS(iGPSPosition,vTimestampsiGPS);
    ...
    SLAM.LoadiGPSPosition(vTimestampsiGPS,iGPSPosition);  //temp function
    ...
    for (seq = 0; seq<num_seq; seq++)
    {
        ...
        for(int ni=0; ni<nImages[seq]; ni++, proccIm++)
        {
            ...
            //cout << "time stamp  " << 1e+9 * tframe << endl;
            if(im.empty())
            {
...
void LoadiGPS(vector<cv::Point3f> &iGPSPosition,vector<double> &vTimestampsiGPS)
{
    ...
    while(!fTimes.eof()) {
        ...
        if (!s.empty()) {
            ...
            ss >> p2;
            ss >> p3;
            cv::Point3f PositionCoor;
            PositionCoor.x = p1;
            PositionCoor.y = p2;
            PositionCoor.z = p3;
            vTimestampsiGPS.push_back(t/1e9);
            iGPSPosition.push_back(PositionCoor);
